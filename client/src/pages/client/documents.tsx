...
const DOC_CONFIG = {
  resume: {
    ...
    placeholder:
      "e.g. Can you emphasize my leadership experience more? Also fix the typo in the first paragraph...",
  },
  "cover-letter": {
    color: "purple",
    label: "Cover Letter",
    ...
    placeholder:
      "e.g. Can you adjust the opening paragraph to mention my passion for fintech? Make the closing stronger...",
  },
  linkedin: {
    color: "blue",
    ...
    placeholder:
      "e.g. Can you make the headline more catchy? Add more keywords to the skills section for SEO...",
  },
};
...
export default function ClientDocumentsPage() {
    ...
    if (clientData) {
      ...
      try {
        const saved = localStorage.getItem(
          `client_approvals_${currentUser?.id}`,
        );
        if (saved) {
  ...
  const [documentFeedback, setDocumentFeedback] = useState<
    Record<DocType, { text: string; status: "requested" | "completed" | null }>
  >({
  ...
  const [revealPhase, setRevealPhase] = useState<
    "intro" | "float" | "distort" | "explode" | "reveal" | "settle"
  >("intro");
  ...
  const [unlockedDocs, setUnlockedDocs] = useState<Record<DocType, boolean>>(
    () => {
      ...
      try {
        const saved = localStorage.getItem(
          `client_unlocked_${currentUser?.id}`,
        );
        return saved
          ? JSON.parse(saved)
          : {
  ...
  const handleSubmitRevisions = () => {
    ...
    toast.success("Revisions Requested", {
      description:
        "Your team has been notified and will update your documents.",
    });
  };
  ...
  const handleImprove = () => {
    ...
    const fileKey =
      activeTab === "resume"
        ? "resume_improved"
        : activeTab === "cover-letter"
          ? "cover_letter_A" // Defaulting to A for now
  ...
  const handleReviewClick = () => {
    ...
    try {
      localStorage.setItem(
        `client_unlocked_${currentUser?.id}`,
        JSON.stringify(newUnlockedDocs),
      );
    } catch (e) {
  ...
  const handleApprove = () => {
    ...
    if (isRealClientId) {
      ...
      // Fallback to localStorage for mock users
      try {
        const existingApprovalsStr = localStorage.getItem(
          `client_approvals_${currentUser?.id}`,
        );
        const existingApprovals = existingApprovalsStr
          ? JSON.parse(existingApprovalsStr)
          : {};
      ...
      {beforePdfUrl ? (
        ...
        <div className="flex flex-col items-center justify-center h-full min-h-[600px] w-full text-gray-400">
          ...
          <p className="text-center max-w-md text-gray-500">
            Your original {config.label.toLowerCase()} will appear here once
      ...
      {afterPdfUrl ? (
        ...
        <div className="flex flex-col items-center justify-center h-full min-h-[600px] w-full text-gray-400">
          ...
          <p className="text-center max-w-md text-gray-400">
            Your improved {config.label.toLowerCase()} is being prepared by your
            team.
          </p>
        </div>
      )}
  ...
  return (
    <div className="space-y-6 h-[calc(100vh-6rem)] flex flex-col relative overflow-hidden">
            ...
            <div className="relative w-full h-full flex flex-col items-center justify-center overflow-hidden">
              ...
              <AnimatePresence>
                {["intro", "float", "distort"].includes(revealPhase) && (
                  <motion.h2
                    className={cn(
                      "text-4xl md:text-6xl font-bold text-white tracking-widest uppercase z-20 absolute",
                      config.text,
                    )}
                  ...
                  >
                    ...
                    {false ? (
                      ...
                      <div className="space-y-4 w-3/4 opacity-40 blur-[1px]">
                        <h3 className="text-xl font-bold text-center mb-4 text-gray-500">
                          Old {config.label}
                        </h3>
                    ...
                    <motion.div
                      className={cn(
                        "absolute inset-0 rounded blur-xl opacity-50",
                        config.bg,
                      )}
                ...
                <div className="absolute inset-0 pointer-events-none z-50 flex items-center justify-center">
                  ...
                  {[...Array(50)].map((_, i) => (
                    <motion.div
                      key={i}
                      className={cn(
                        "absolute rounded-full w-2 h-2",
                        i % 2 === 0 ? config.bg : "bg-white",
                      )}
                      initial={{
                        x: 0,
                        y: 0,
            ...
            >
              ...
              <div className="p-6 border-t bg-gray-50 flex justify-center">
                <Button
                  size="lg"
                  className={cn(
                    "h-12 px-8 text-lg font-bold shadow-lg hover:scale-105 transition-transform",
                    config.bg,
                    "text-white",
                  )}
                  onClick={handleReviewClick}
      ...
      <div className="flex-1 min-h-0 flex flex-col">
        <Tabs
          value={activeTab}
          onValueChange={(v) => setActiveTab(v as DocType)}
            ...
            {(Object.keys(DOC_CONFIG) as DocType[])
              .filter((tabKey) => {
                ...
                if (hasApplications && tabKey === "linkedin") {
                  const hasLinkedInDoc =
                    uploadedFiles["linkedin_A"] ||
                    uploadedFiles["linkedin_improved"];
                ...
                <AnimatePresence mode="wait">
                  ...
                  {hasApplications ? (
                    <motion.div
                      ...
                      className={cn(
                        "absolute inset-0 bg-[#111] text-white shadow-2xl rounded-lg overflow-hidden flex flex-col",
                      )}
                    >
                      <div className="bg-[#111] border-b border-white/10 p-4 flex justify-between items-center shrink-0">
                        <div className="flex items-center gap-4">
                          <div
                            className={cn(
                              "text-white px-4 py-1 rounded font-bold uppercase tracking-wider text-sm flex items-center gap-2",
                              config.bg,
                            )}
                          >
                        ...
                        <div className="flex items-center gap-2">
                          {(isFlipped ? beforePdfUrl : afterPdfUrl) && (
                            <Button
                              variant="outline"
                              size="sm"
                    ...
                    >
                      ...
                      <div
                        className={cn(unlockedDocs[activeTab] ? "mt-16" : "")}
                      >
                        ...
                        {!beforePdfUrl ? (
                          <div className="flex flex-col items-center justify-center h-full min-h-[600px] w-full text-gray-400">
                            ...
                            <p className="text-center max-w-md">
                              Your {config.label.toLowerCase()} is being
                              prepared by your team. You'll be notified when
                    ...
                    >
                      <div className="bg-[#111] border-b border-white/10 p-4 flex justify-between items-center shrink-0">
                        ...
                        <div className="flex items-center gap-2">
                          ...
                          {documentFeedback[activeTab]?.status ===
                            "requested" && (
                    ...
                    {hasApplications ? (
                      <>
                        ...
                        <div>
                          ...
                          <p className="text-sm text-muted-foreground mt-2">
                            These are the documents being used for your job
                            applications.
                          </p>
                        </div>
                      </>
                      ...
                      !hasImprovedDocument ? (
                        ...
                        <>
                          ...
                          <Button
                            size="lg"
                            disabled
                            className="w-full font-bold text-lg h-12 bg-white/5 text-gray-500 cursor-not-allowed"
                          >
                            Not Yet Available
                          </Button>
                        </>
                        ...
                        <>
                          <div
                            className={cn(
                              "p-4 rounded-full mb-2 animate-pulse bg-white/10",
                              config.text,
                            )}
                          >
                          ...
                          <div>
                            <h3 className="text-xl font-bold text-white">
                              {unlockedDocs[activeTab]
                                ? `This is your old ${config.label.toLowerCase()}`
                                : `Your New ${config.label} Is Ready`}
                            </h3>
                            <p className="text-sm text-muted-foreground mt-2">
                              {activeTab === "resume"
                                ? "We've crafted a high-impact resume optimized for ATS."
                                : activeTab === "cover-letter"
                                  ? "Three versions tailored for different role types."
                          ...
                          <Button
                            size="lg"
                            className={cn(
                              "w-full font-bold text-lg h-12 shadow-lg hover:scale-105 transition-transform",
                              config.bg,
                              "text-white",
                            )}
                            onClick={
                      ...
                      <>
                        ...
                        <div>
                          ...
                          <p className="text-sm text-muted-foreground mt-2">
                            We are working on your updates. You'll be notified
                        ...
                        <Button
                          variant="outline"
                          className="w-full mt-4 border-white/10 opacity-50 cursor-not-allowed"
                        >
                          Awaiting Updates
                        </Button>
                      </>
                      ...
                      <>
                        ...
                        <div>
                          ...
                          <p className="text-sm text-muted-foreground mt-2">
                            Request revisions or approve this version.
                          </p>
                        </div>
                        ...
                        <div className="w-full space-y-3 pt-4">
                          ...
                          >
                            <Check className="w-5 h-5 mr-2" /> Approve This
                            Version
                          </Button>
                          <Button
                            variant="outline"
                            className="w-full border-white/10 hover:bg-white/5"
                            onClick={handleStartRevisionRequest}
                          >
                            Request Revisions
                          </Button>
                        </div>
                      ...
                      documentFeedback[activeTab]?.status !== "requested" && (
                        <div className="absolute inset-0 z-20 bg-[#111] p-6 flex flex-col animate-in fade-in duration-200">
                          ...
                          <Textarea
                            ...
                            onChange={(e) =>
                              setRevisionRequestText(e.target.value)
                            }
                            autoFocus
                          />
                          ...
                          <div className="space-y-3">
                            ...
                            <Button
                              ...
                              onClick={handleCancelRevisionRequest}
                            >
                              Cancel
                            </Button>
                          </div>
