...
export default function ClientInterviewsPage() {
  ...
  const downloadPdf = async (app: Application) => {
    ...
    try {
      ...
      container.innerHTML = `
        <div style="font-family: 'Helvetica', 'Arial', sans-serif; padding: 40px; color: #333; max-width: 800px;">
          <div style="border-bottom: 3px solid #3b82f6; padding-bottom: 20px; margin-bottom: 30px;">
            <h1 style="margin: 0 0 8px 0; font-size: 28px; color: #1a1a1a;">Interview Prep</h1>
            <h2 style="margin: 0 0 4px 0; font-size: 20px; color: #3b82f6; font-weight: 600;">${app.company_name}</h2>
            <p style="margin: 0; font-size: 16px; color: #666;">${app.job_title}</p>
          </div>
          <div style="font-size: 14px; line-height: 1.7; white-space: pre-wrap;">${(app as any).prep_doc || "No prep document available."}</div>
          <div style="margin-top: 40px; padding-top: 20px; border-top: 1px solid #e5e5e5; font-size: 12px; color: #999;">
            Generated by Jumpseat • ${format(new Date(), "MMMM d, yyyy")}
          </div>
        </div>
      `;
