...
export default function AdminInterviewsPage() {
  ...
  // Get selected client name for copy
  const selectedClient = clients.find((c: Client) => c.id === selectedClientId);

  // Copy job info for research agent
  const copyForResearch = async (app: Application) => {
    const clientName = selectedClient
      ? `${selectedClient.first_name} ${selectedClient.last_name}`
      : "Unknown Client";

    const textToCopy = `Interview Prep Research Request

Client: ${clientName}
Company: ${app.company_name}
Position: ${app.job_title}
Job URL: ${app.job_url || "N/A"}

Please research this company and role to prepare interview talking points.`;
