import type {
  ...
  JobCriteriaSample,
  InsertJobCriteriaSample,
  UpdateJobCriteriaSample,
  ClientJobResponse,
  InsertClientJobResponse,
  ApplierJobSession,
  InsertApplierJobSession,
  UpdateApplierJobSession,
  FlaggedApplication,
  InsertFlaggedApplication,
...
export interface IStorage {
  ...
  updateJobSample(
    id: string,
    updates: UpdateJobCriteriaSample,
  ): Promise<JobCriteriaSample | null>;
  ...
  createJobResponse(
    response: InsertClientJobResponse,
  ...
  getApplierSessionByJob(
    jobId: string,
    applierId: string,
  ): Promise<ApplierJobSession | null>;
  createApplierSession(
    session: InsertApplierJobSession,
  ): Promise<ApplierJobSession>;
  updateApplierSession(
    id: string,
    updates: UpdateApplierJobSession,
  ): Promise<ApplierJobSession | null>;

  // Flagged application operations
  getFlaggedApplications(): Promise<FlaggedApplication[]>;
  getFlaggedApplicationsByStatus(
    status: "open" | "resolved",
  ): Promise<FlaggedApplication[]>;
  createFlaggedApplication(
    flagged: InsertFlaggedApplication,
  ): Promise<FlaggedApplication>;
  updateFlaggedApplication(
    id: string,
    updates: UpdateFlaggedApplication,
  ): Promise<FlaggedApplication | null>;

  // Applier earnings operations
  getApplierEarnings(applierId: string): Promise<ApplierEarning[]>;
  getApplierEarningsByDateRange(
    applierId: string,
    startDate: string,
    endDate: string,
  ): Promise<ApplierEarning[]>;
  createApplierEarning(earning: InsertApplierEarning): Promise<ApplierEarning>;
  updateApplierEarning(
    id: string,
    updates: UpdateApplierEarning,
...
export class SupabaseStorage implements IStorage {
  ...
  async updateClient(
    ...
    if (
      Array.isArray(cleanedUpdates.target_job_titles) &&
      (cleanedUpdates.target_job_titles as string[]).length === 0
    ) {
      cleanedUpdates.target_job_titles = null;
    }
    if (
      Array.isArray(cleanedUpdates.required_skills) &&
      (cleanedUpdates.required_skills as string[]).length === 0
    ) {
      cleanedUpdates.required_skills = null;
    }
    if (
      Array.isArray(cleanedUpdates.nice_to_have_skills) &&
      (cleanedUpdates.nice_to_have_skills as string[]).length === 0
    ) {
      cleanedUpdates.nice_to_have_skills = null;
    }
    if (
      Array.isArray(cleanedUpdates.exclude_keywords) &&
      (cleanedUpdates.exclude_keywords as string[]).length === 0
    ) {
      cleanedUpdates.exclude_keywords = null;
    }
    if (
      Array.isArray(cleanedUpdates.seniority_levels) &&
      (cleanedUpdates.seniority_levels as string[]).length === 0
    ) {
  ...
  async getQueueJobs(clientId: string, applierId: string): Promise<Job[]> {
    ...
    return jobs.filter(
      (job) => !appliedJobIds.has(job.id) && !flaggedJobIds.has(job.id),
    );
  }
  ...
  async deleteJob(id: string): Promise<void> {
    ...
    await supabase.from("applier_job_sessions").delete().eq("job_id", id);
  ...
  async getAppliers(): Promise<Applier[]> {
    const { data, error } = await supabase.from("appliers").select("*");
  ...
  async createJobSample(
    sample: InsertJobCriteriaSample,
  ...
  async getFlaggedApplicationsByStatus(
    status: "open" | "resolved",
  ): Promise<FlaggedApplication[]> {
    const { data, error } = await supabase
      .from("flagged_applications")
  ...
  async createFlaggedApplication(
    flagged: InsertFlaggedApplication,
  ): Promise<FlaggedApplication> {
    const { data, error } = await supabase
      .from("flagged_applications")
  ...
  async updateFlaggedApplication(
    id: string,
    updates: UpdateFlaggedApplication,
  ): Promise<FlaggedApplication | null> {
    const { data, error } = await supabase
  ...
  async getApplierEarningsByDateRange(
    applierId: string,
    startDate: string,
    endDate: string,
  ): Promise<ApplierEarning[]> {
  ...
  async createApplierEarning(
    earning: InsertApplierEarning,
  ): Promise<ApplierEarning> {
    const earningData = {
      ...earning,
      earned_date:
        earning.earned_date || new Date().toISOString().split("T")[0],
    };
